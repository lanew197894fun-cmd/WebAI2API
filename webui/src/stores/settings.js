  ...
  actions: {
    ...
    async handleResponse(res, successMsg) {
      ...
      if (res.ok) {
        ...
        // 後端返回格式: { error: { message: "..." } } 或 { message: "..." }
        const errorMessage =
          data.error?.message ||
          data.message ||
          `請求未成功: ${res.status} ${res.statusText}`;
        Modal.error({
          title: "保存失敗",
    ...
    async saveServerConfig(config) {
      try {
        ...
        const result = await this.handleResponse(res, "伺服器設定保存成功");
        if (result.success) {
    ...
    async saveBrowserConfig(config) {
      try {
        ...
        const result = await this.handleResponse(res, "瀏覽器設定保存成功");
        if (result.success) {
    ...
    async saveWorkerConfig(config) {
      try {
        ...
        const result = await this.handleResponse(res, "實例配置保存成功");
        if (result.success) {
    ...
    async savePoolConfig(config) {
      try {
        ...
        const result = await this.handleResponse(res, "工作池設定保存成功");
        if (result.success) {
