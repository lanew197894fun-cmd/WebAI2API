...
const fetchQueue = async () => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const processing = (data.processingTasks || []).map((t) => ({
        ...t,
        status: "processing",
      }));
      const waiting = (data.waitingTasks || []).map((t) => ({
        ...t,
        status: "waiting",
      }));
...
<template>
  <a-layout style="width: 100%; background: transparent">
    <!-- 安全模式告警横幅 -->
    <a-alert
      v-if="systemStore.safeMode?.enabled"
      type="error"
      show-icon
        ...
        <a-card title="系统状态" :bordered="false" style="height: 100%">
          <a-space direction="vertical" style="width: 100%" size="middle">
            ...
            <div>
              ...
              <a-progress
                :percent="systemStore.cpuUsage"
                :stroke-color="getLoadColor(systemStore.cpuUsage)"
                :show-info="false"
              />
            </div>
            ...
            <div>
              ...
              <a-progress
                :percent="
                  Math.round(
                    (systemStore.memoryUsage.used /
                      systemStore.memoryUsage.total) *
                      100,
                  ) || 0
                "
                :stroke-color="
