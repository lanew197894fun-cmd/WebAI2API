...
export class PoolManager {
  ...
  async initAll() {
    ...
    if (loginArg && loginArg.includes("=")) {
      ...
      logger.info(
        "工作池",
        `登录模式: 仅初始化第一个 Worker "${loginWorkerName}"`,
      );
    }
    ...
    if (isLoginMode) {
      logger.info(
        "工作池",
        `登录模式: 从 ${workerConfigs.length} 个 Worker 中筛选...`,
      );
    } else {
    ...
    for (const workerConfig of workerConfigs) {
      if (isLoginMode && workerConfig.name !== loginWorkerName) {
        logger.debug("工作池", `[${workerConfig.name}] 跳過 (不匹配登录目标)`);
        continue;
      }

      if (
        workerConfig.type !== "merge" &&
        !registry.hasAdapter(workerConfig.type)
      ) {
        logger.error(
          "工作池",
          `Worker [${workerConfig.name}] 的類型 "${workerConfig.type}" 无对应配接器，跳過`,
        );
        continue;
      }
      ...
      if (workerConfig.type === "merge") {
        ...
        if (invalidTypes.length > 0) {
          logger.error(
            "工作池",
            `Worker [${workerConfig.name}] 的 mergeTypes 包含无效類型: ${invalidTypes.join(", ")}`,
          );
          continue;
        }
      }
    ...
    for (const worker of validWorkers) {
      try {
        ...
        if (existing) {
          ...
          logger.debug(
            "工作池",
            `[${worker.name}] 将与其他 Worker 共享瀏覽器 (${worker.userDataDir})`,
          );
          await worker.init(existing.browser);

          // 建立共享关系：設定所有者引用，并添加到所有者的共享列表
          ...
          browserMap.set(worker.userDataDir, {
            ...
            ownerWorker: worker, // 儲存所有者 Worker 引用
          });
        }

        this.workers.push(worker);
      } catch (e) {
        logger.error("工作池", `[${worker.name}] 初始化失败，跳過该 Worker`, {
          error: e.message,
        });
      }
    }
    ...
    if (this.workers.length === 0) {
      throw new Error("所有 Worker 初始化都失败了，无法啟動服务");
    }

    this.initialized = true;
    logger.info(
      "工作池",
      `工作池初始化完成，共 ${this.workers.length} 个 Worker 就绪 (${browserMap.size} 个瀏覽器实例)`,
    );
  }
  ...
  selectWorker(modelId) {
    ...
    if (candidates.length === 0) {
      throw new Error(`没有 Worker 支援模型: ${modelId}`);
    }
  ...
  async generate(ctx, prompt, paths, modelId, meta) {
    ...
    if (candidates.length === 0) {
      return { error: `没有 Worker 支援模型: ${modelId}` };
    }

    // 如果请求包含圖片，优先选择 imagePolicy 为 optional 的 Worker
    ...
    if (hasImages && candidates.length > 1) {
      ...
      if (optionalCandidates.length > 0) {
        logger.debug(
          "工作池",
          `请求包含圖片，优先选择支援圖片的 Worker (${optionalCandidates.length}/${candidates.length} 个)`,
        );
        candidates = optionalCandidates;
      } else {
        logger.warn(
          "工作池",
          `请求包含圖片，但没有 Worker 的 imagePolicy 为 optional`,
        );
      }
    }
    ...
    if (!failoverEnabled) {
      ...
      logger.debug(
        "工作池",
        `任务分发至: ${worker.name} (busy: ${worker.busyCount})`,
      );
      return await this._safeExecuteWorker(
        worker,
        ctx,
        prompt,
        paths,
  ...
  async _safeExecuteWorker(worker, ctx, prompt, paths, modelId, meta) {
    try {
      ...
      logger.error("工作池", `[${worker.name}] 執行异常`, {
        error: err.message,
        ...meta,
      });
      return normalizeError(err.message || "執行异常");
    }
  }
  ...
  /**
   * 取得圖片策略（宽松策略：只要有一个 Worker 支援 optional 就回傳 optional）
   */
  ...
  async getCookies(instanceName, domain) {
    ...
    if (instanceName) {
      ...
      if (!worker) {
        throw new Error(`瀏覽器实例不存在: ${instanceName}`);
      }
    } else {
