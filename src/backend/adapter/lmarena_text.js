...
async function generate(context, prompt, imgPaths, modelId, meta = {}) {
  ...
  try {
    logger.info("配接器", `开启新会话... (搜索模式: ${!!search})`, meta);
    ...
    for (const line of lines) {
      if (line.startsWith("a0:")) {
        try {
          ...
          logger.warn("配接器", `解析文本块失败: ${line}`, meta);
        }
        ...
        try {
          ...
          logger.warn("配接器", `解析思考块失败: ${line}`, meta);
        }
      }
    }
...
export const manifest = {
  ...
  description:
    "使用 LMArena 平台生成文本，支援多种大语言模型和搜索模式。需要已登录的 LMArena 账户，若不登录会频繁弹出人机验证码且有速率限制。",
