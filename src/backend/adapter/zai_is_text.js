...
async function handleDiscordAuth(page) {
  ...
  if (currentUrl.includes("zai.is/auth")) {
    ...
    logger.info(
      "配接器",
      "[登录器(zai)] 偵測到登录页面，正在处理 Discord 登录...",
    );
    ...
    try {
      ...
      logger.info("配接器", "[登录器(zai)] 已到达 Discord 授权页面");
      ...
      for (let i = 0; i < 15; i++) {
        ...
        if (authorizeBtn) {
          const isDisabled = await authorizeBtn
            ...
            .catch(() => true);
          if (!isDisabled) {
            logger.info("配接器", "[登录器(zai)] 授权按钮已可用，正在點擊...");
            await sleep(300, 500);
      ...
      logger.info("配接器", "[登录器(zai)] 等待跳转回目标页面...");
      await page.waitForURL(
        (url) => {
          const href = url.href;
          return (
      ...
      logger.info("配接器", "[登录器(zai)] Discord 登录完成");
      ...
      logger.warn(
        "配接器",
        `[登录器(zai)] Discord 登录处理失败: ${err.message}`,
      );
      unlockPageAuth(page);
    }
  }
...
function extractTextContent(content) {
  ...
  const detailsPattern =
    /^<details\s+type="reasoning"[^>]*>[\s\S]*?<\/details>\s*/;

  // 移除开头的 details 块，回傳剩余内容
...
async function generate(context, prompt, imgPaths, modelId, meta = {}) {
  ...
  try {
    ...
    // 2. 上傳圖片 (如果有多张圖片，会一张一张上傳，每次都是 v1/files POST 请求)
    ...
    // 6. 等待 v1/chats/new 回應 (状态码 200 且回應体中有 id)
    ...
    // 7. 等待 chat/completions 回應 (状态码 200 且 status: true)
    ...
    // 8. 等待 chat/completed 回應，从中提取文本内容
    ...
    if (!textContent) {
      logger.warn("配接器", "提取文本内容为空", meta);
      return { error: "提取文本内容为空" };
    }

    logger.info("配接器", `已提取文本内容 (${textContent.length} 字元)`, meta);
...
export const manifest = {
  ...
  description:
    "使用 zAI 平台生成文本，支援多种大语言模型。需要 Discord 账户登录授权。",
