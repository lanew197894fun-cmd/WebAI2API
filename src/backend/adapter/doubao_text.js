/**
 * @fileoverview 豆包 (Doubao) 文本生成配接器
 */
...
async function generate(context, prompt, imgPaths, modelId, meta = {}) {
  ...
  const MODEL_MENU_MAP = {
    ...
    "seed-pro":
      /Pro Advanced Pro model|专家 研究级智能模型|專家 研究級智慧模型/,
  };
  ...
  try {
    ...
    logger.debug(
      "配接器",
      `选择模型: ${modelId} -> ${String(modelMenuName)}`,
      meta,
    );
    await sleep(300, 500);
    ...
    if (selectorExists) {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        try {
          ...
          logger.warn("配接器", `模型菜单未弹出，重試 ${attempt}/3`, meta);
          if (attempt === 3) throw new Error("模型选择菜单未弹出");
        }
      }
      await safeClick(page, menuItem, { bias: "button" });
      await sleep(600, 1000); // 留出充足时间等待模型选择浮窗自动關閉，防止遮挡上傳图标
    }
    ...
    if (imgPaths && imgPaths.length > 0) {
      ...
      try {
        // 點擊上傳菜单按钮（排除掉含有模型名称或带有“更多”文案的按钮）
        const uploadMenuBtn = page
          .locator('#input-engine-container button[aria-haspopup="menu"]')
          .filter({
            hasNot: page.locator(
              "text=/Fast|Think|Pro|快速|思考|专家|專家|更多/",
            ),
          })
          .first();
        ...
        const uploadItem = page.locator('div[role="menuitem"]').filter({
          hasText: /上傳檔案或圖片|上傳檔案或圖片|Upload File or Image/,
        });
        ...
        logger.error("配接器", `圖片上傳失败: ${uploadErr.message}`, meta);
        // 不抛出异常，继续尝试发送纯文本
      } finally {
    ...
    const resultPromise = new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        if (!isResolved) {
          ...
          reject(
            new Error(
              `API_TIMEOUT: 回應逾時 (${Math.round(waitTimeout / 1000)}秒)`,
            ),
          );
        }
      }, waitTimeout);
      ...
      const handleResponse = async (response) => {
        try {
          const url = response.url();
          // 只处理 chat/completion 介面的 SSE 回應
    ...
    if (resultText) {
      logger.info("配接器", `生成完成，文本长度: ${resultText.length}`, meta);
...
export const manifest = {
  ...
  displayName: "豆包 (文本生成)",
  description:
    "使用字节跳动豆包生成文本，支援深度思考模式和圖片上傳。需要已登录的豆包账户。",
