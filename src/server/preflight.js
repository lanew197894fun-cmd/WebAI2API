/**
 * @fileoverview 服务器啟動前自检模組
 * @description 检查补丁、Camoufox 可執行檔案、version.json、GeoLite2 数据库和 better-sqlite3 是否就绪。
 */
 ...
 * @returns {string|null} MD5 哈希值，檔案不存在回傳 null
 */
...
export function preflight() {
  ...
  if (!fs.existsSync(sqlitePath)) {
    errors.push("better-sqlite3 预编译檔案缺失，請執行: npm run init");
  }
  ...
  for (const [patchName, targetName] of Object.entries(CAMOUFOX_PATCHES)) {
    ...
    if (patchHash !== targetHash) {
      errors.push("camoufox-js 补丁未应用，請執行: pnpm install");
      break; // 只报告一次
    }
  }
...
export function runPreflight() {
  ...
  if (!result.ok) {
    ...
    logger.error(
      "服务器",
      "提示: 您可以使用 npm run init -- -custom 来自定义初始化步骤",
    );
    // 退出码 78 表示配置/依赖錯誤，看门狗不应自动重启
    process.exit(78);
  }
