/**
 * @fileoverview Admin API 路由模組
 * @description 提供管理介面，包括系统状态、配置管理、配接器元数据等
 */
 ...
 * @param {object} context.config - 完整配置物件
 * @param {object} context.queueManager - 佇列管理器
 * @param {string} context.tempDir - 临时目錄
 * @returns {Function} Admin 路由处理函式
 */
export function createAdminRouter(context) {
   ...
   * @param {string} pathname - 去除 /admin 前缀后的路徑
   */
  return async function handleAdminRequest(req, res, pathname) {
    ...
    try {
      ...
      // DELETE /admin/logs - 清除系统日誌
      if (method === "DELETE" && pathname === "/logs") {
        const success = logger.clearLogs();
        if (success) {
          logger.info("管理器", "系统日誌已清除");
          sendJson(res, 200, { success: true, message: "日誌已清除" });
        } else {
          sendJson(res, 500, { success: false, message: "日誌清除失败" });
        }
        return;
      }

      // GET /admin/data-folders - 列出数据檔案夹
      ...
      // POST /admin/data-folders/delete - 刪除数据檔案夹
      ...
      // GET/POST /admin/config/pool - 負載均衡和故障转移配置
      ...
      // GET /admin/adapters - 取得配接器列表（含 configSchema）
      ...
      // GET /admin/stats/range - 查询日期範圍统计
      ...
      // DELETE /admin/stats/range - 刪除日期範圍统计
      ...
      // GET /admin/history/models - 取得历史中使用过的模型列表
      ...
      // GET /admin/history/media/:filepath - 静态媒体檔案服务
      ...
      // POST /admin/history/:id/retry-media - 重試下載媒體
      const retryMediaMatch = pathname.match(
        /^\/history\/([^/]+)\/retry-media$/,
      );
