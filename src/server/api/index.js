 ...
 * @param {boolean} [context.loginMode] - 登录模式（停用 OpenAI API）
 * @returns {Function} 请求处理函式
 */
export function createGlobalRouter(context) {
  const { authToken, config, queueManager, tempDir, loginMode, getSafeMode } =
    context;
    ...
    if (pathname.startsWith("/v1")) {
      ...
      if (safeMode?.enabled) {
        ...
        res.end(
          JSON.stringify({
            error: {
              message: `服務執行在安全模式，OpenAI API 不可用。原因: ${safeMode.reason}`,
      ...
      if (!handleOpenAIRequest) {
        ...
        res.end(
          JSON.stringify({
            error: {
              message: "服務執行在登錄模式，OpenAI API 不可用",
              type: "service_unavailable",
            },
          }),
        );
