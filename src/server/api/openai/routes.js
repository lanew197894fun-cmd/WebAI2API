 ...
 * @description 处理 /v1 路徑下的所有 API 请求
 */
...
export function createOpenAIRouter(context) {
  ...
  async function handleCookies(res, requestId, workerName, domain) {
    ...
    try {
      ...
      logger.error("服务器", "取得 Cookies 失败", {
        id: requestId,
        error: err.message,
      });

      if (
        err.message.includes("Worker 不存在") ||
        err.message.includes("Worker not found")
      ) {
        sendApiError(res, {
  ...
  async function handleChatCompletions(req, res, requestId) {
    ...
    try {
      ...
      if (!isStreaming && !queueManager.canAcceptNonStreaming()) {
        ...
        logger.warn("服务器", "非流式请求被拒绝 (佇列已满)", {
          id: requestId,
          queueSize: status.total,
        });
        sendApiError(res, {
          code: ERROR_CODES.SERVER_BUSY,
          message: `服务器繁忙（佇列: ${status.total}/${queueManager.maxQueueSize}）。请使用流式模式 (stream: true) 或稍後重試。`,
        });
        return;
      }
      ...
      logger.info("服务器", `[佇列] 请求入队: ${prompt.slice(0, 100)}...`, {
        id: requestId,
   ...
   * @param {string} pathname - 去除 /v1 前缀后的路徑
   * @param {URL} parsedUrl - 解析后的 URL 物件
   */
