/**
 * @fileoverview 请求解析模組
 * @description 负责解析聊天请求、提取提示詞和处理圖片
 */
 ...
 * @property {string} prompt - 提取的提示詞
 * @property {string[]} imagePaths - 圖片临时檔案路徑
 ...
 * @property {string} code - 錯誤码
 * @property {string} error - 錯誤消息
 */
 ...
 * @property {ParseError} [error] - 錯誤資訊（失败时）
 */
 ...
 * @param {string} options.tempDir - 临时目錄路徑
 * @param {number} options.imageLimit - 圖片数量限制
 * @param {string} options.backendName - 后端名称
 * @param {Function} options.getSupportedModels - 取得支援的模型列表函式
 * @param {Function} options.getImagePolicy - 取得圖片策略函式
 * @param {Function} options.getModelType - 取得模型類型函式
 * @param {string} options.requestId - 请求 ID
 * @param {Function} options.logger - 日誌函式
  ...
  if (data.model) {
    ...
    if (isSupported) {
      ...
      return parseError(
        ...
        `模型无效/后端 ${backendName} 不支援: ${data.model}`,
      );
    }
  } else {
    logger.info("服务器", "未指定模型，使用网页預設", { id: requestId });
  }
 ...
 * 解析文本请求 (构建虚拟上下文)
 */
async function parseTextRequest(
  messages,
  tempDir,
  imageLimit,
  modelId,
      ...
      for (const item of content) {
        if (item.type === "text") {
          ...
          if (imageLimit > 0 && globalImageCount > imageLimit) {
            textBuffer += `[圖片${globalImageCount} (已忽略:超过限制)]`;
            continue;
          }
          ...
          if (url.startsWith("data:image")) {
            ...
            if (imagePath) {
              ...
              // 插入占位符
              textBuffer += `[圖片${globalImageCount}]`;
            } else {
              textBuffer += `[圖片${globalImageCount} (上傳失敗)]`;
            }
          } else {
            textBuffer += `[圖片${globalImageCount} (无效链接)]`;
          }
        }
      }
    }
        ...
        if (imageLimit <= 10) {
          if (imageCount > imageLimit) {
            return parseError(
              ...
              `圖片数量超过限制（最大 ${imageLimit} 张）`,
            );
          }
        } else {
          // imageLimit > 10：超过瀏覽器硬限制时忽略
          if (imageCount > 10) {
            continue;
          }
        }
 ...
 * @param {string} tempDir - 临时目錄
 * @returns {Promise<string|null>} 儲存的檔案路徑，失败回傳 null
 */
