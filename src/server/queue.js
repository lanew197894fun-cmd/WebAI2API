/**
 * @fileoverview 任务佇列管理模組
 * @description 负责请求佇列、並發控制和心跳機制，配接 Pool 模式架構
 */
...
import {
  createRecord,
  updateRecord,
  processResponseMedia,
} from "../utils/history.js";

/**
 * @typedef {object} TaskContext
 * @property {import('http').IncomingMessage} req - HTTP 请求物件
 * @property {import('http').ServerResponse} res - HTTP 回應物件
 * @property {string} prompt - 用戶提示詞
 * @property {string[]} imagePaths - 圖片路徑列表
 ...
 * @property {number} maxConcurrent - 最大並發数
 * @property {number} maxQueueSize - 最大佇列大小
 ...
 * @property {object} config - 配置物件
 */

/**
 * 建立任务佇列管理器
 * @param {QueueConfig} queueConfig - 佇列配置
 * @param {object} callbacks - 回调函式
 * @param {Function} callbacks.initBrowser - 初始化 Pool 函式
 * @param {Function} callbacks.generate - 生成圖片函式
 * @param {object} callbacks.config - 配置物件
 * @param {Function} [callbacks.navigateToMonitor] - 監控导航函式
 * @param {Function} [callbacks.getCookies] - 取得 Cookies 函式
 * @returns {object} 佇列管理器
 */
export function createQueueManager(queueConfig, callbacks) {
  const { maxConcurrent, queueBuffer, keepaliveMode } = queueConfig;
  const { initBrowser, generate, config, navigateToMonitor, getCookies } =
    callbacks;

  // 计算有效佇列大小：0 表示不限制，否则为 maxConcurrent + buffer
  const effectiveQueueSize =
    queueBuffer === 0 ? Infinity : maxConcurrent + queueBuffer;
  ...
  async function processTask(task) {
    ...
    try {
      ...
      logger.debug("服务器", `建立历史记录失败: ${e.message}`);
    }
    ...
    try {
      ...
      processResponseMedia(result, id)
        ...
        .catch((e) => {
          logger.debug("服务器", `处理回應媒体失败: ${e.message}`);
        });

      // 发送成功回應
      logger.info("服务器", "准备发送回應...", {
        id,
        isStreaming,
  ...
  async function getWorkerCookies(workerName, domain) {
    if (!getCookies) {
      throw new Error("getCookies 回调未註冊");
    }
