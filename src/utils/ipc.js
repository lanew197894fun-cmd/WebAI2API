...
export async function sendRestartSignal(extraArgs = []) {
  ...
  if (!ipcPath) {
    console.warn("[IPC] 未執行在 Supervisor 模式下，无法发送重启信号");
    return false;
  }
  ...
  return new Promise((resolve) => {
    const client = net.createConnection(ipcPath, () => {
      ...
      const command =
        extraArgs.length > 0 ? `RESTART:${extraArgs.join(" ")}` : "RESTART";
    ...
    client.on("error", (err) => {
      console.error("[IPC] 連接 Supervisor 失败:", err.message);
      resolve(false);
    });
  });
}
...
export async function sendStopSignal() {
  ...
  if (!ipcPath) {
    console.warn("[IPC] 未執行在 Supervisor 模式下，无法发送停止信号");
    return false;
  }
