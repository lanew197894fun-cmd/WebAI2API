/**
 * @fileoverview 系统資訊工具模組
 * @description 提供系统状态、数据檔案夹管理等功能
 */
...
/**
 * 刪除指定的数据檔案夹
 * @param {string[]} folderNames - 要刪除的檔案夹名称列表
...
export function deleteDataFolders(folderNames, workers = []) {
  ...
  for (const name of folderNames) {
    // 安全检查：只允许刪除 camoufoxUserData 开头的檔案夹
    if (!name.startsWith("camoufoxUserData")) {
      errors.push(`${name}: 不允许刪除非用戶数据檔案夹`);
      continue;
    }

    // 检查是否正在使用
    if (inUse.has(name)) {
      errors.push(`${name}: 檔案夹正在被 Worker 使用`);
      continue;
    }
...
export function clearTempFiles(tempDir) {
  ...
  try {
    ...
    logger.info("系统", `已清理 ${cleaned} 个临时檔案`);
  } catch (e) {
    logger.warn("系统", `清理临时檔案失败: ${e.message}`);
  }
